...
let characterRotationAngle1 = 0;
let characterRotationAngle2 = 0;
let characterRotationAngle3 = 0;
...
function drawCharacter(x, y) {
  ...
  push();
    ...
    translate(750,500);
    rotate(radians(characterRotationAngle1));
      characterRotationAngle1 += 2;

    hyunscale = ((sin(a1 = a1 + 0.01)+0.4));
    scale(hyunscale);
    push();
...
function pikachudraw() {
  ...
  push();
    translate(700,800);
    rotate(radians(characterRotationAngle2));
      characterRotationAngle2 += 50;

    anscale = ((sin(a2 = a2 + 0.3)+0.2));
    scale(anscale);
    push();
        translate(-200,-300)
        ellipseMode(CENTER);
...
function drawSpongeBob(x,y) {
  ...
  push();
    translate(200,500);
    rotate(radians(-characterRotationAngle3));
      characterRotationAngle3 += 1;

    jinscale = ((sin(a3 = a3 + 0.02)+0.2));
    scale(jinscale);
...
**20171111 김종준 (귀여운 피카츄) :** p5.js를 사용하여 피카츄를 그리기 위해서 Chatgpt나 뤼튼등의 GPT모델의 도움을 좀 받았습니다. 코드를 통해 그림을 그리고 수정하는 과정에서 문제 해결 능력이 향상되었고, 원하는 결과물을 얻기 위해 여러 기술을 시도해보는 과정에서 성취감을 느낄 수 있었습니다. 또한, 코드로 그림을 만드는것은 포토샵 등으로 작품을 평소에 만들던 저에게는 새로운 예술 형태에 도전하는 기회가 되었습니다. p5.js의 여러 소스와 Example이 존재하고 소스의 세상을 탐험하고 그것을 참고하는 일 또한 너무 즐거운 일이었습니다.

**20210817 신현진 (정장의 사나이) :** 이번과제 2D 캐릭터 만들면서 많은것들을 추가하고 싶은 욕심이 많이 생겼습니다. 그래서 이것저것 추가하다 보니 원인을 알수없는 오류로 인한 스트레스를 받기도했고 코드를 만드는데 거의 하루를 사용한거 같았습니다. 하지만 이러한 원인들을 스스로 해결하면서 쾌감을 느꼈고 자신이 성장하고 있음을 다시한번 느끼게된 계기가 된거같습니다. 비록 이러한 간단한 캐릭터를 만드는데도 하루를 사용했지만 p5.js에 있는 쇼케이스 탭에서의 여러 작품처럼 훌륭한 작품하나를 만들수 있을때까지 열심히 하겠습니다. 그리고 다같이 고민하며 해보니 현재는 그리 주석이 없어도 알아볼수있었지만 좀더 복잡해진다면 주석의 존재는 필연적인 존재가 될것이라 느꼈습니다. 이번 팀프로젝트로 좀더 남의 입장에서도 생각을 해봐야한다는것을 느꼈습니다. 감사합니다!
